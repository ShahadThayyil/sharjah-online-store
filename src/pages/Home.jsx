...
import { SearchX, Search, SlidersHorizontal, ChevronLeft, ChevronRight } from "lucide-react";
import gsap from "gsap";

const DEFAULT_CATEGORIES = ["Toys", "Phone Cases", "Accessories", "Stationery", "Other"];
const PRODUCTS_PER_PAGE = 20; // <-- Added: Maximum products per page
...
export default function Home() {
    ...
    // Pagination State
    const [currentPage, setCurrentPage] = useState(1); // <-- Added: Track current page
    ...
    useEffect(() => {
        ...
        setFiltered(result);
        setCurrentPage(1); // <-- Added: Reset to Page 1 whenever filters change
    }, [searchQuery, activeCategory, appliedPrice, sortOrder, products]);

    // ─── PAGINATION CALCULATION ───────────────────────────────────────────────
    const indexOfLastProduct = currentPage * PRODUCTS_PER_PAGE;
    const indexOfFirstProduct = indexOfLastProduct - PRODUCTS_PER_PAGE;
    // This is the array we actually pass to the grid now (max 20 items)
    const currentProducts = filtered.slice(indexOfFirstProduct, indexOfLastProduct);
    const totalPages = Math.ceil(filtered.length / PRODUCTS_PER_PAGE);

    const handlePageChange = (pageNumber) => {
        setCurrentPage(pageNumber);
        // Smooth scroll up to the grid when changing pages
    ...
    useEffect(() => {
        const ctx = gsap.context(() => {
            ...
            tl.fromTo(heroImageRef.current.children,
                { x: 100, opacity: 0, rotation: 5 },
                { x: 0, opacity: 1, rotation: 0, duration: 1.2, stagger: 0.2 },
                "-=0.8"
            );
    ...
    // Animate grid when loading finishes OR when changing pages
    useEffect(() => {
        if (!loading && gridRef.current) {
            gsap.fromTo(gridRef.current.querySelectorAll('.product-card-wrapper'),
                { y: 40, opacity: 0 },
                { y: 0, opacity: 1, duration: 0.6, stagger: 0.05, ease: "power2.out" }
            );
        }
    }, [loading, currentProducts]); // <-- Changed: Now watches currentProducts instead of filtered
    ...
    return (
        <div className="min-h-screen bg-[#F4F4F5] text-[#18181B] overflow-x-hidden selection:bg-[#C8102E] selection:text-white pt-24 md:pt-32" style={bodyStyle}>
            ...
            <div className="max-w-[1500px] mx-auto px-4 sm:px-6 pb-8 md:pb-16">
                ...
                <div className="flex flex-col lg:flex-row items-center justify-between gap-12 lg:gap-8 mb-20 md:mb-32 min-h-[50vh]">
                    <div ref={heroTextRef} className="w-full lg:w-1/2 flex flex-col items-start z-10 lg:pb-12">
                        ...
                        <p className="text-gray-500 text-lg md:text-xl max-w-md leading-relaxed mb-10">
                            Premium mobile gear and lifestyle accessories curated for minimalists. Function meets flawless design.
                        </p>
                        <button 
                        onClick={scrollToProducts}
                        className="group relative px-8 py-4 bg-[#18181B] text-white rounded-full font-semibold overflow-hidden transition-all duration-300 hover:shadow-xl hover:shadow-[#C8102E]/20">
                    ...
                    <div ref={heroImageRef} className="w-full lg:w-1/2 relative h-[450px] md:h-[550px] flex justify-center lg:justify-end">
                        <img src="https://images.unsplash.com/photo-1611186871348-b1ce696e52c9?auto=format&fit=crop&q=80&w=800" alt="Featured Tech" className="absolute right-0 top-0 w-[85%] md:w-[75%] h-[85%] object-cover rounded-[2rem] shadow-2xl z-10" />
                        <img src="https://images.unsplash.com/photo-1583394838336-acd977736f90?auto=format&fit=crop&q=80&w=600" alt="Lifestyle Accent" className="absolute left-0 bottom-0 w-[55%] md:w-[45%] h-[55%] object-cover rounded-[2rem] shadow-2xl z-20 border-4 border-[#F4F4F5]" />
                    </div>
                </div>

                {/* ─── MODERN CONTROL PANEL FILTERS ──────────────────────────────── */}
                <div ref={filterRef} className="bg-white/60 backdrop-blur-xl border border-white rounded-3xl p-4 md:p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] mb-16">
                ...
                <div ref={gridRef} className="scroll-mt-32">
                    {loading ? (
                        ...
                        <div className="py-24 text-center max-w-md mx-auto bg-white rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100">
                            ...
                            <p className="text-gray-500 mb-6 text-sm">We couldn't find any gear matching those exact filters.</p>
                        ...
                        // Passing currentProducts (max 20) instead of all filtered products
                    ...
                    <div className="mt-20 flex items-center justify-center gap-2">
                        ...
                        <button 
                            ...
                            className={`p-3 rounded-full flex items-center justify-center transition-all duration-300 ${
                                ...
                                : "text-[#18181B] bg-white hover:bg-[#18181B] hover:text-white shadow-sm hover:shadow-md"
                            }`}
                        >
                            <ChevronLeft size={20} />
                        </button>
                            ...
                            {Array.from({ length: totalPages }, (_, i) => i + 1).map((number) => (
                                <button
                                    ...
                                    className={`w-10 h-10 flex items-center justify-center rounded-full text-sm font-bold transition-all duration-300 ${
                                        ...
                                        : "text-gray-500 hover:text-[#18181B] hover:bg-gray-100"
                                    }`}
                                >
                                    {number}
                                </button>
